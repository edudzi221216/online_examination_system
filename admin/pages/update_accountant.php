<?php
$teacher_id = $_POST['teacher_id'];	// this id is fetched from accountant.php page
...
if ($result->num_rows > 0) {
	...
	if ($tem == $email) {		// if the email is same
	 ...
	 window.location.href='../edit-accountant.php?Duplicate Email ID Found!!&tid=$teacher_id';
	 </script>";	
	}else{
    ...
    window.location.href='../edit-accountant.php?Duplicate Phone Number Found!!!&tid=$teacher_id';
    </script>";		
	}else{
...
if ($conn->query($sql) === TRUE) {
  echo "<script>
    alert('Accountant updated successfully. ACCOUNTANT ID: '+'$teacher_id');
    window.location.href='../accountant.php?Added Successfully';
    </script>";
} else {
  echo "<script>
    alert('Could not add accountant.');
    window.location.href='../edit-accountant.php?Could Not Apply Settings&tid=$teacher_id';
    </script>";
}
